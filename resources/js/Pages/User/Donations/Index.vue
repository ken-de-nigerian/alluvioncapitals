<script setup lang="ts">
    ...
    const performSearch = debounce(() => {
        router.get(route('user.payments.donations'), { search: search.value }, {
            preserveState: true,
            replace: true,
            preserveScroll: true
        });
...
<template>
    <Head :title="`${page.props.app.name} | Donations`" />
        ...
        <table class="table align-middle fs-sm mb-4">
            ...
            <tr v-if="donations.data.length === 0">
                <td class="py-3 ps-0 pt-4 pb-4" colspan="5">
                    <h2 class="h6 pt-2 mb-2">No donations received yet</h2>
                    <p class="fs-sm mb-4" style="max-width: 640px">
                        You haven't received any donations so far. Share your campaign with friends and supporters to start receiving contributions.
                    </p>

                    <Link :href="route('user.campaigns.index')" class="btn btn-dark">
                        <i class="ci-share fs-base ms-n1 me-2"></i>
                        Share campaign
                    </Link>
                </td>
            </tr>
