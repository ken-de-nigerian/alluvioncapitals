<script setup lang="ts">
    ...
    const performSearch = debounce(() => {
        router.get(route('admin.categories.show', props.category.slug), { search: search.value }, {
            preserveState: true,
            replace: true,
            preserveScroll: true
        });
    ...
    const toggleSelectAll = () => {
        if (selectAll.value) {
            selectedItems.value = props.campaigns.data.map((category: any) => category.id);
        } else {
            selectedItems.value = [];
        }
    };
    ...
    const bulkArchive = () => {
        if (selectedItems.value.length === 0) {
            showToast('error', "Please select rows to archive.");
            return;
        }

        confirmAction("Are you sure you want to archive the selected items?", () => {
            router.post(route('admin.campaigns.archive'), { ids: selectedItems.value }, {
                preserveScroll: true,
                onSuccess: () => {
                    selectedItems.value = [];
    ...
    const markAsComplete = () => {
        if (selectedItems.value.length === 0) {
            showToast('error', "Please select rows to mark as complete.");
            return;
        }

        confirmAction("Ending this campaign is final and cannot be undone. Do you wish to continue?", () => {
            router.post(route('admin.campaigns.complete'), {
...
<template>
    ...
    <div class="col-lg-9">
        ...
        <div class="overflow-auto mb-3">
            <ul class="nav nav-pills flex-nowrap gap-2 text-nowrap pb-3" role="tablist">
                <li class="nav-item" role="presentation">
                    <Link :href="route('admin.categories.show', props.category.slug)" :class="{ 'active': page.component === 'Admin/Category/Show/Published' }" class="nav-link">
                ...
                <li class="nav-item" role="presentation">
                    <Link :href="route('admin.categories.show.archived', props.category.slug)" :class="{ 'active': page.component === 'Admin/Category/Show/Archived' }" class="nav-link">
        ...
        <div class="tab-content">
            <div class="tab-pane fade" :class="{ 'show active': page.component === 'Admin/Category/Show/Published' }">
                <!-- Loading state -->
                        ...
                        <div class="col-12 col-md-8">
                            <div class="d-flex flex-column flex-md-row align-items-start align-items-md-center flex-wrap">
                                ...
                                <div class="d-flex flex-wrap" :class="{ 'd-none': selectedItems.length === 0 }" id="published-action-buttons">
                                    <button class="nav-link position-relative px-0 pe-sm-2 py-2 me-4" @click="markAsComplete">
                                        <i class="ci-close-octagon fs-base me-2"></i>
                                        <span class="hover-effect-underline d-md-inline">
                                                Mark as complete (<span id="archiveCount">{{ selectedItems.length }}</span>)
                                            </span>
                                    </button>

                                    <button class="nav-link position-relative px-0 pe-sm-2 py-2 me-4" @click="bulkArchive">
                                        <i class="ci-archive fs-base me-2"></i>
                                        <span class="hover-effect-underline d-md-inline">
                                                Move to archive (<span id="archiveCount">{{ selectedItems.length }}</span>)
                                            </span>
                                    </button>
                                                ...
                                                <div v-if="campaign.status_badge.text !== 'Completed' && campaign.days_left_text !== 'Expired'" class="d-flex justify-content-end gap-2 mb-3">
                                                    ...
                                                    <div class="dropdown">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-menu-end">
                                                            <li>
                                                                <Link class="dropdown-item" :href="route('admin.campaigns.rewards.index', campaign.id)">
                                                                    <i class="ci-gift opacity-75 me-2"></i> Gifts
                                                                </Link>
                                                            </li>

                                                            <li>
                                                                <Link class="dropdown-item" :href="route('admin.campaigns.updates.index', campaign.id)">
                                                                    <i class="ci-corner-right-up fs-base opacity-75 me-2"></i> Updates
                                                                </Link>
                                                            </li>

                                                            <li>
                                                                <Link class="dropdown-item" :href="route('admin.campaigns.comments.index', campaign.id)">
