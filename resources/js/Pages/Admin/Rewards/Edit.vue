<script setup lang="ts">
    ...
    const form = useForm({
        ...
        amount: props.reward?.amount || '',
        description: props.reward?.description || '',
        status: props.reward?.status || '',
        physical_gift: props.reward?.requires_shipping || false,
    });

    const submit = () => {
        form.post(route('admin.campaigns.rewards.update', { reward: props.reward.id }), {
            preserveScroll: true,
            onSuccess: () => {
                form.reset();
            }
...
<template>
    <Head :title="`${page.props.app.name} | Account - Edit Reward`"/>
    ...
    <div class="col-lg-9 pt-2 pt-xl-3">
        ...
        <form @submit.prevent="submit">
            ...
            <div class="col-md-12 position-relative pb-2 mb-3 mb-sm-4">
                ...
                <select class="form-select form-select-lg" :class="{ 'is-invalid': form.errors.status }" v-model="form.status" id="status" @focus="form.clearErrors('status')">
