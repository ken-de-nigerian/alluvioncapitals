<script setup lang="ts">
    ...
    const form = useForm({
        ...
        phone_number: props.old?.phone_number || '',
        country: props.old?.country || '',
        country_id: props.old?.country_id || '',
        state: props.old?.state || '',
        state_id: props.old?.state_id || '',
        city: props.old?.city || '',
    ...
    const submit = () => {
        ...
        form.transform(() => submitData).post(route('campaigns.add.contact.info.store'), {
            preserveScroll: true,
        });
    };
...
<template>
    <Head :title="`${page.props.app.name} | Account - Campaigns: Add Contact Info`" />
    ...
    <footer class="sticky-bottom bg-body pb-3">
        ...
        <div class="container d-flex gap-3 pt-3">
            <Link class="btn btn-outline-dark animate-slide-start" :href="route('campaigns.add.details')">
