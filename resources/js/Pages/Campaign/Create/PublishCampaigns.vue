<script setup lang="ts">
    ...
    onMounted(() => {
        ...
        // Initialize with the featured plan selected by default
        ...
        planCards.value.forEach(card => {
            card.addEventListener('click', function(e: Event) {
                // Don't trigger if clicking on the button (let the button handler do it)
                const target = e.target as HTMLElement;
                if (!target.classList.contains('select-plan') && !target.closest('.select-plan')) {
                    selectPlan(card);
                }
            });
        ...
        addonSwitches.value.forEach(switchEl => {
            switchEl.addEventListener('change', function() {
                ...
                if (this.checked) {
                    ...
                    selectedAddons.value = selectedAddons.value.filter(item => item.addon !== addon);
                }
    ...
    function updateTotal() {
        ...
        if (totalAmountElement.value) {
            totalAmountElement.value.textContent = totalAmount.value.toLocaleString('en-US', {
    ...
    function submit() {
        ...
        // Submit the form
        form.post(route('campaigns.add.publish.live'), {
            preserveScroll: true,
            onFinish: () => {
