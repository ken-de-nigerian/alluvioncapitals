<script setup lang="ts">
    ...
    const form = useForm({
        ...
        campaign_video: props.media?.campaign_video || props.campaign?.campaign_video || '',
    });
    ...
    // Submit form
    const submit = () => {
        form.post(route('campaigns.edit.photos.videos.store', props.campaign?.id), {
            preserveScroll: true,
            forceFormData: true,
            onError: (errors) => {
    ...
    onMounted(() => {
        // First, try to get from media prop
        let images = props.media?.campaign_images;

        // If not found, try campaign prop
        if (!images) {
            images = props.campaign?.campaign_images;
        }
...
<template>
    ...
    <footer class="sticky-bottom bg-body pb-3">
        ...
        <div class="container d-flex gap-3 pt-3">
            <Link class="btn btn-outline-dark animate-slide-start" :href="route('campaigns.edit.contact.info', props.campaign?.id)">
