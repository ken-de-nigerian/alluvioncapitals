<script setup lang="ts">
    ...
    const form = useForm({
        title: props.old?.title || props.campaign?.title || '',
        category_id: props.old?.category_id || props.campaign?.category_id || '',
        goal: props.old?.goal || props.campaign?.goal || '',
        expires_at: props.old?.expires_at || props.campaign?.expires_at || new Date().toISOString().split('T')[0],
        summary: props.old?.summary || props.campaign?.summary || '',
    });

    const submit = () => {
        form.post(route('campaigns.edit.details.store', props.campaign?.id), {
            preserveScroll: true,
        });
    };
...
<template>
    <Head :title="`${page.props.app.name} | Account - Campaigns: Edit Campaign Details`" />
