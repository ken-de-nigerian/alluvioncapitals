...
class ForgotPasswordController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if (config('settings.email_notification')) {
            Mail::mailer(config('settings.email_provider'))->to($user->email)->send(new PasswordResetLink($resetUrl));
        }
    ...
    public function update(Request $request): RedirectResponse
    {
        ...
        // Send email
        if (config('settings.email_notification')) {
            Mail::mailer(config('settings.email_provider'))->to($user->email)->send(new PasswordResetConfirmation($user));
        }
